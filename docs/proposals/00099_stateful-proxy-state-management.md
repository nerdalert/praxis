...
features should use explicit storage classes and typed
...
demos, and explicitly single-replica deployments. Shared
hot-path implementations should use a Redis/Valkey
compatible backend first, with strict timeouts, TTLs, key
conventions, failure-mode defaults, and bounded metrics
labels.
...
- Do not require every stateful feature to use Redis.
...
Praxis is becoming an AI-native proxy framework, not just
...
Redis/Valkey is a good first shared hot-path backend
