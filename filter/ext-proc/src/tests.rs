...
#![allow(
    ...
    reason = "standard test patterns; too_many_lines for inline mock processors"
)]
#![allow(
    ...
    reason = "inline mock structs inside test functions require items_after_statements; Arc clones and doc backticks are ubiquitous in mock setup"
)]
...
// -----------------------------------------------------------------------------
// Body Mode Rejection Contract
// -----------------------------------------------------------------------------

#[tokio::test]
async fn accepts_request_body_full_duplex_streamed() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
...
"#,
    ...
    assert_eq!(
        filter.name(),
        "ext_proc",
        "full_duplex_streamed request body mode should be accepted"
    );
}

#[tokio::test]
async fn rejects_request_body_streamed() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
...
"#,
    ...
    assert!(
        ...
        "error should mention request_body_mode: {msg}"
    );
    assert!(msg.contains("streamed"), "error should name the rejected mode: {msg}");
}

#[tokio::test]
async fn rejects_request_body_buffered() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
...
"#,
    ...
    assert!(msg.contains("buffered"), "error should name the rejected mode: {msg}");
}

#[tokio::test]
async fn rejects_request_body_buffered_partial() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
...
"#,
    ...
    assert!(
        msg.contains("buffered_partial"),
        "error should name the rejected mode: {msg}"
    );
}

#[tokio::test]
async fn rejects_response_body_full_duplex_streamed() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
...
"#,
    ...
    let msg = err.to_string();
    assert!(
        msg.contains("response_body_mode"),
        "error should mention response_body_mode: {msg}"
    );
    assert!(
        msg.contains("full_duplex_streamed"),
        "error should name the rejected mode: {msg}"
    );
    assert!(
        msg.contains("not yet supported"),
        "error should indicate mode is not yet supported: {msg}"
    );
}

#[tokio::test]
async fn rejects_response_body_streamed() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
...
#[tokio::test]
async fn rejects_response_body_buffered() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
...
#[tokio::test]
async fn rejects_response_body_buffered_partial() {
    let yaml: serde_yaml::Value = serde_yaml::from_str(
        r#"
target: "http://127.0.0.1:50051"
processing_mode:
            ...
            match behavior {
                ...
                DuplexBehavior::DelayedRouting {
                    ...
                    loop {
                        ...
                        if let Some(processing_request::Request::RequestBody(ref b)) = body_msg.request
                            && b.end_of_stream
                        {
                            break;
                        }
...
async fn duplex_first_message_includes_protocol_config() {
    ...
    let mut exchange = ExtProcExchange::open(channel, &config, None).unwrap();
...
async fn duplex_second_message_omits_protocol_config() {
    ...
    impl ExternalProcessor for AllConfigRecorder {
        ...
        #[allow(clippy::significant_drop_tightening, reason = "lock must span the loop body")]
        async fn process(
            &self,
            request: tonic::Request<tonic::Streaming<ProcessingRequest>>,
        ) -> Result<tonic::Response<Self::ProcessStream>, tonic::Status> {
    ...
    let channel = connect_channel(addr).await;
    let mut exchange = ExtProcExchange::open(channel, &streamed_body_exchange_config(), None).unwrap();
    exchange.send(make_request_headers()).await.unwrap();
...
async fn duplex_request_headers_round_trip() {
    ...
    .await;
    let channel = connect_channel(addr).await;
    let mut exchange = ExtProcExchange::open(channel, &default_exchange_config(), None).unwrap();
    exchange.send(make_request_headers()).await.unwrap();
...
async fn duplex_delayed_routing_no_deadlock() {
    ...
    .await;
    let channel = connect_channel(addr).await;
    let mut exchange = ExtProcExchange::open(channel, &full_duplex_exchange_config(), None).unwrap();
...
fn exchange_is_send_and_sync() {
    ...
    assert_send::<ExtProcState>();
    assert_sync::<ExtProcState>();
}
...
async fn duplex_drop_exchange_cleans_up() {
    let (addr, _guard) = start_duplex_processor(DuplexBehavior::Hang).await;
    let channel = connect_channel(addr).await;
    let exchange = ExtProcExchange::open(channel, &default_exchange_config(), None).unwrap();
    drop(exchange);
}
...
async fn duplex_concurrent_exchanges_no_crosstalk() {
    ...
    impl ExternalProcessor for EchoIdProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                while let Ok(Some(msg)) = stream.message().await {
                    if let Some(processing_request::Request::RequestHeaders(ref h)) = msg.request {
                        let id_header = h
                            .headers
                            .as_ref()
...
async fn duplex_repeated_clean_close() {
    ...
    for i in 0..20 {
        let mut exchange = ExtProcExchange::open(shared_channel.clone(), &default_exchange_config(), None).unwrap();
        exchange.send(make_request_headers()).await.unwrap();
        let resp = exchange.receive().await.unwrap();
        assert!(
            matches!(resp, ExchangeEvent::RequestHeaders { .. }),
...
use praxis_proto::envoy::service::ext_proc::v3::TrailersResponse;
...
async fn duplex_override_ignored_in_full_duplex() {
    ...
    impl ExternalProcessor for FullDuplexOverrideProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                let _headers = stream.message().await.unwrap().unwrap();
                // In full-duplex, headers create no active state,
                // so this override envelope has no timer to modify.
                // It is consumed and ignored because
    ...
    impl ExternalProcessor for DoubleOverrideProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                ...
                // Second override: override_consumed is true, so this
                // is an invalid override. Because override_message_timeout
                // is present, the entire envelope is consumed and ignored.
...
async fn duplex_zero_duration_override_ignored() {
    ...
    impl ExternalProcessor for ZeroOverrideProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                let msg = stream.message().await.unwrap().unwrap();
                // Zero-duration override: rejected (< 1ms minimum).
                // Because override_message_timeout is present, the
                // entire envelope is consumed and ignored.
...
async fn duplex_backpressure_deterministic() {
    ...
    impl ExternalProcessor for BarrierProcessor {
        ...
        async fn process(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel(16);
            tokio::spawn(async move {
                // Read first message, then block on barrier to stop
                // consuming from the h2 stream.
                let _msg = stream.message().await.unwrap().unwrap();
                barrier.wait().await;
                while let Ok(Some(msg)) = stream.message().await {
                    let resp = build_noop_response(&msg);
    ...
    // Send large body chunks until backpressure blocks. The bounded
    // mpsc channel (capacity REQUEST_CHANNEL_CAPACITY) feeds tonic's
    // h2 driver, which has its own flow control window (~64 KiB
    // default). Use 16 KiB chunks so h2 flow control kicks in after
    // a few sends.
    let chunk_size = 16_384; // 16 KiB
    let big_chunk = vec![0xABu8; chunk_size];
...
async fn duplex_unsolicited_response_rejected() {
    ...
    impl ExternalProcessor for UnsolicitedResponseProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                let _msg = stream.message().await.unwrap().unwrap();
                // Server sends a ResponseHeaders response before any
                // ResponseHeaders was sent by the client. This is
...
async fn duplex_full_duplex_trailers_while_deferred() {
    ...
    impl ExternalProcessor for DeferredTrailerProcessor {
        ...
        async fn process(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel(16);
            tokio::spawn(async move {
                // Read all messages first: headers, body chunks, trailers.
    ...
    // Send headers, body chunks, trailers -- all before receiving
    // any response. In full-duplex mode, no active state blocks
    // any of these sends.
    exchange.send(make_request_headers()).await.unwrap();
    exchange.send(make_request_body(b"chunk1", false)).await.unwrap();
    ...
    impl ExternalProcessor for StreamedInNonFdProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                ...
                let _body = stream.message().await.unwrap().unwrap();
                // Send a StreamedBodyResponse in non-full-duplex
                // (Streamed) mode — this must be rejected.
                use praxis_proto::envoy::service::ext_proc::v3::{
                    BodyMutation, CommonResponse, StreamedBodyResponse, body_mutation,
                };
                let body_resp = ProcessingResponse {
    ...
    impl ExternalProcessor for NonStreamedInFdProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                ...
                let _body = stream.message().await.unwrap().unwrap();
                // Send a non-streamed body response (ordinary body
                // bytes, no StreamedBodyResponse mutation) in
                // full-duplex mode — this must be rejected.
                let body_resp = ProcessingResponse {
                    response: Some(processing_response::Response::RequestBody(BodyResponse {
                        response: None,
                    })),
...
async fn duplex_rejected_response_does_not_advance_output_phase() {
    ...
    // Capture output phases after rejection.
...
async fn duplex_deadline_overflow_returns_error_not_panic() {
    ...
    let result = exchange.send(make_request_headers()).await;
    assert!(
        matches!(result, Err(ExchangeError::OrderingViolation(ref msg)) if msg.contains("overflow")),
...
async fn duplex_cancelled_blocked_send_leaves_state_unchanged() {
    ...
    // Start a blocked send with a unique body, then cancel it
    // by letting the future go out of scope.
    {
        ...
        // Future dropped here — cancelled send.
    }

    // Release capacity by receiving the fill message.
    let first = rx.recv().await.unwrap();
    // Verify it is the fill message, not the cancelled one.
    assert!(
    ...
    // Channel must be empty — cancelled message was never committed.
    assert!(
        rx.try_recv().is_err(),
        "channel should be empty after removing the fill message; cancelled message must not be present"
    );

    // Send a follow-up through the same channel.
    ...
    // Verify the receiver gets the follow-up, not the cancelled message.
...
async fn duplex_repeated_close_with_eof_count() {
    ...
    // Allow time for server-side EOF detection.
    tokio::time::sleep(Duration::from_millis(200)).await;
    ...
    // Final exchange on same channel succeeds.
    let mut final_exchange = ExtProcExchange::open(shared_channel.clone(), &default_exchange_config(), None).unwrap();
...
async fn duplex_cross_direction_started_non_fd_duplicate_body_rejected() {
    ...
    impl ExternalProcessor for WrongTypeProcessor {
        ...
        #[allow(clippy::too_many_lines, reason = "multi-phase mock")]
        async fn process(
            &self,
            request: tonic::Request<tonic::Streaming<ProcessingRequest>>,
        ) -> Result<tonic::Response<Self::ProcessStream>, tonic::Status> {
...
async fn duplex_blocked_send_deadline_starts_after_commit() {
    ...
    // Assert it remains pending for longer than message_timeout.
    ...
    // The deadline should be approximately now + 200ms (not 250ms ago + 200ms).
...
// =============================================================================
// FD03 Integration Tests — Full-Duplex Request Flow
// =============================================================================
...
async fn full_duplex_headers_sent_before_body() {
    // Verify that headers are sent to the processor before body
    // chunks, even when on_request_body is called first (pre-read).
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                while let Ok(Some(msg)) = stream.message().await {
                    ...
                    // Defer all responses until body EOS. Accumulate
                    // labels so we can verify order after the test.
                    if label_str == "body_eos" {
                        // Respond to headers first, then body EOS.
                        let header_resp = ProcessingResponse {
                            response: Some(processing_response::Response::RequestHeaders(HeadersResponse {
                                response: None,
                            })),
...
"#,
    ...
    // Simulate pre-read: on_request_body is called before on_request.
    ...
    // Then on_request is called — idempotent.
    let action = filter.on_request(&mut ctx).await.unwrap();
    assert!(
        matches!(action, FilterAction::Continue),
...
async fn full_duplex_body_chunks_sent_incrementally() {
    // Processor that records all body chunks received, waits for
    // body EOS, then responds to headers and body in order.
    ...
    impl ExternalProcessor for ChunkRecordingProcessor {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                ...
                // Read body chunks until EOS.
                loop {
                    let msg = stream.message().await.unwrap().unwrap();
                    if let Some(processing_request::Request::RequestBody(ref b)) = msg.request {
                ...
                // Respond: headers response first.
                let header_resp = ProcessingResponse {
                    response: Some(processing_response::Response::RequestHeaders(HeadersResponse {
                        response: None,
                    })),
    ...
    impl ExternalProcessor for EosBodyRecorder {
        ...
        async fn process(
            ...
            tokio::spawn(async move {
                ...
                // Read body chunks until EOS, recording the EOS body.
                loop {
                    let msg = stream.message().await.unwrap().unwrap();
                    if let Some(processing_request::Request::RequestBody(ref b)) = msg.request
                        && b.end_of_stream
...
"#,
    ...
    // Synthetic EOS — the body parameter may contain the frozen
    // accumulated body but we should send an empty terminal.
...
async fn full_duplex_deferred_routing_response() {
    // Simulates the Go EPP pattern: receives all body data before
    // responding, then sends a header response with endpoint mutation
    // and a body response with the original body.
    let (addr, _guard) = start_duplex_processor(DuplexBehavior::DelayedRouting {
...
"#,
    ...
    // on_request: sends headers, returns Continue.
    let action = filter.on_request(&mut ctx).await.unwrap();
    assert!(
        matches!(action, FilterAction::Continue),
    ...
    // EOS: triggers drain, receives header response with endpoint
    // mutation and body response.
    let mut eos_body = None;
    let action = filter.on_request_body(&mut ctx, &mut eos_body, true).await.unwrap();
    assert!(
        matches!(action, FilterAction::Continue),
    ...
    // Verify the endpoint header mutation was applied.
...
"#,
    ...
    // on_request sends headers. The processor responds with
    // ImmediateResponse, but in full-duplex mode we do not
    // wait for the response in on_request — it returns Continue.
    let action = filter.on_request(&mut ctx).await.unwrap();
    assert!(
        matches!(action, FilterAction::Continue),
    ...
    // EOS triggers drain, which receives the ImmediateResponse.
    let mut eos_body = None;
    let action = filter.on_request_body(&mut ctx, &mut eos_body, true).await.unwrap();
    let rejection = match action {
        FilterAction::Reject(r) => r,
...
"#,
    ...
    // on_request succeeds (headers sent).
    let action = filter.on_request(&mut ctx).await.unwrap();
    assert!(
        matches!(action, FilterAction::Continue),
    ...
    // EOS drain will timeout waiting for responses.
    let mut eos_body = None;
    let action = filter.on_request_body(&mut ctx, &mut eos_body, true).await.unwrap();
    let rejection = match action {
        FilterAction::Reject(r) => r,
...
async fn driver_one_process_invocation() {
    ...
    exchange.send(make_request_headers()).await.unwrap();
    let _resp = exchange.receive().await.unwrap();
    exchange.send(make_request_headers()).await.ok();
    drop(exchange);
...
async fn driver_outbound_half_close_preserves_drain() {
    ...
    assert!(
        matches!(event, ExchangeEvent::Immediate { ref response, .. } if response.status.as_ref().is_some_and(|s| s.code == 403)),
