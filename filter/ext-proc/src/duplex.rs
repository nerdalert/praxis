...
//! Persistent bidirectional `ext_proc` exchange — FD02 state machine.
//!
...
pub(crate) struct ExchangeConfig {
    /// Timeout for the initial gRPC stream handshake.
    #[allow(dead_code, reason = "used by non-preload callers and future timeout restoration")]
...
pub(crate) enum ExchangeError {
    ...
    /// The initial gRPC stream handshake timed out.
    #[allow(dead_code, reason = "used by non-preload callers and future timeout restoration")]
    #[error("ext_proc stream-open timeout")]
    OpenTimeout,
...
pub(crate) enum ExchangeEvent {
    ...
    #[allow(dead_code, reason = "classified by receive; consumed in FD04+")]
    RequestTrailers {
        /// Processor response payload.
...
impl ExtProcExchange {
    ...
    /// [`receive`] first drives the pending future. No open timeout
    /// is enforced here; the bounded lifecycle timeout at drain
    /// covers the coalesced path.
    ///
    /// [`send`]: Self::send
    /// [`receive`]: Self::receive
    #[allow(clippy::too_many_lines, reason = "exchange initialization with all state fields")]
    pub(crate) fn open(
        channel: Channel,
        config: &ExchangeConfig,
        first_message: Option<ProcessingRequest>,
    ) -> Result<Self, ExchangeError> {
        let (tx, rx) = mpsc::channel(REQUEST_CHANNEL_CAPACITY);

        let has_preload = first_message.is_some();
    ...
    #[allow(
        ...
        reason = "transactional send with validation, reserve, commit, state update"
    )]
        ...
        let checked_deadline = timeout
            .map(|dur| {
                tokio::time::Instant::now()
                    .checked_add(dur)
                    .ok_or(ExchangeError::OrderingViolation("deadline overflow".to_owned()))
            })
            .transpose()?;
    ...
    /// Commit the state transition for a message that was
    /// pre-loaded into the channel during [`open`].
    ///
    /// The message was already sent, so no I/O occurs. This
    /// method only computes and applies the state transition,
    /// setting up active processing and send-phase tracking.
    ///
    /// [`open`]: Self::open
    pub(crate) fn commit_preloaded_send(&mut self, request: &processing_request::Request) -> Result<(), ExchangeError> {
        let transition = self.compute_send_transition(request)?;
        let checked_deadline = transition
            .active_state
            .map(|_| {
                tokio::time::Instant::now()
                    .checked_add(self.message_timeout)
                    .ok_or_else(|| ExchangeError::OrderingViolation("deadline overflow".to_owned()))
            })
            .transpose()?;
        self.apply_send_transition(&transition, checked_deadline);
        Ok(())
    ...
    #[allow(dead_code, reason = "used by integration tests and FD04+")]
    ...
    #[allow(clippy::too_many_lines, reason = "select! branches with state transitions")]
        ...
        let transition = match request {
            processing_request::Request::RequestHeaders(_) => {
                ...
                SendTransition {
                    ...
                    active_state: if creates_active {
                        Some(ExpectedResponse::RequestHeaders)
                    } else {
                        None
                    },
                }
            ...
            processing_request::Request::RequestTrailers(_) => {
                ...
                SendTransition {
                    ...
                    active_state: if creates_active {
                        Some(ExpectedResponse::RequestTrailers)
                    } else {
                        None
                    },
                }
            ...
            processing_request::Request::ResponseHeaders(_) => {
                ...
                SendTransition {
                    ...
                    new_phase: SendPhase::Headers,
                    active_state: if creates_active {
                        Some(ExpectedResponse::ResponseHeaders)
                    } else {
                        None
                    },
                }
            ...
            processing_request::Request::ResponseTrailers(_) => {
                ...
                SendTransition {
                    ...
                    new_phase: SendPhase::Trailers,
                    active_state: if creates_active {
                        Some(ExpectedResponse::ResponseTrailers)
                    } else {
                        None
                    },
                }
...
impl ExtProcExchange {
    ...
    ///
    /// 1. Reads a response with optional deadline from active processing.
    /// 2. Runs the override loop: if `override_message_timeout` is present on the envelope, it is an override envelope.
    ...
    /// 3. Classifies the response against expected type and validates output ordering.
    ///
    /// [`classify_and_validate`]: Self::classify_and_validate
    #[allow(
        clippy::too_many_lines,
        reason = "override loop with multi-condition validation before classification"
    )]
    ...
    fn classify_and_validate(&mut self, resp: ProcessingResponse) -> Result<ExchangeEvent, ExchangeError> {
        ...
        match response {
            ...
            processing_response::Response::RequestBody(ref r) => {
                ...
                self.request_output = local_output;
                self.consume_active_if_matched(expected);
                Ok(ExchangeEvent::RequestBody {
                    response: match response {
                        processing_response::Response::RequestBody(r) => r,
                        _ => unreachable!(),
                    },
                    metadata,
                })
            ...
            processing_response::Response::ResponseBody(ref r) => {
                ...
                self.response_output = local_output;
                self.consume_active_if_matched(expected);
                Ok(ExchangeEvent::ResponseBody {
                    response: match response {
                        processing_response::Response::ResponseBody(r) => r,
                        _ => unreachable!(),
                    },
                    metadata,
                })
    ...
    #[allow(
        clippy::too_many_lines,
        reason = "cross-direction and full-duplex validation branches"
    )]
...
#[allow(dead_code, reason = "used by deterministic send tests")]
