...
struct TrustedPeerConfig {
    ...
    ///
    /// Weaker than certificate digest — useful for bootstrap
    /// and demo configurations where cert digests are not
    /// known ahead of time.
...
/// bootstrap or demo configurations where cert digests are not
/// known ahead of time. SAN/SPIFFE identity matching is planned
/// for a follow-up.
///
/// # YAML configuration
///
...
impl GridIngressTrustFilter {
    ...
    pub fn from_config(config: &serde_yaml::Value) -> Result<Box<dyn HttpFilter>, FilterError> {
        ...
        if cfg.trusted_peers.is_empty() {
            return Err("grid_ingress_trust: trusted_peers must not be empty".into());
        }
        if cfg.trusted_peers.len() > MAX_TRUSTED_PEERS {
            return Err(format!("grid_ingress_trust: trusted_peers exceeds maximum of {MAX_TRUSTED_PEERS}").into());
        }
...
impl HttpFilter for GridIngressTrustFilter {
    ...
    async fn on_request(&self, ctx: &mut HttpFilterContext<'_>) -> Result<FilterAction, FilterError> {
        let Some(identity) = &ctx.peer_identity else {
            tracing::warn!("grid ingress trust: no peer identity; rejecting");
        ...
        if is_trusted(identity, &self.trusted_peers) {
            ...
            tracing::warn!(
                peer_digest = %identity.hex_digest(),
                "grid ingress trust: peer not trusted; rejecting"
            );
...
fn validate_peers(raw: Vec<TrustedPeerConfig>) -> Result<Vec<TrustedPeer>, FilterError> {
    ...
    for (i, p) in raw.into_iter().enumerate() {
        ...
        if p.cert_digest.is_none() && p.organization.is_none() && p.serial_number.is_none() {
            return Err(format!("grid_ingress_trust: trusted_peers[{i}] must specify at least one match field").into());
        }
...
fn validate_cert_digest_field(name: &str, value: &Option<String>) -> Result<(), FilterError> {
    ...
    if v.len() != SHA256_HEX_DIGEST_LEN || !v.bytes().all(|b| matches!(b, b'0'..=b'9' | b'a'..=b'f')) {
        return Err(format!("grid_ingress_trust: {name} must be exactly 64 lowercase hex characters").into());
    }
...
fn validate_optional_field(name: &str, value: &Option<String>) -> Result<(), FilterError> {
    ...
    {
        return Err(format!("grid_ingress_trust: {name} must be 1-{MAX_FIELD_LEN} non-blank characters").into());
    }
    Ok(())
}
...
mod tests {
    ...
    fn parse(yaml: &str) -> Result<Box<dyn HttpFilter>, FilterError> {
        ...
        GridIngressTrustFilter::from_config(&val)
    }
