...
pub(super) fn validate_config(cfg: &ModelRewriteConfig) -> Result<(), FilterError> {
    ...
    validate_body_bytes(cfg.max_body_bytes)?;
...
/// Validate `max_body_bytes` bounds.
fn validate_body_bytes(max_body_bytes: usize) -> Result<(), FilterError> {
    if max_body_bytes == 0 {
        return Err("openai_responses_model_rewrite: 'max_body_bytes' must be greater than zero".into());
    }
    if max_body_bytes > MAX_JSON_BODY_BYTES {
        return Err(format!(
            "openai_responses_model_rewrite: max_body_bytes ({max_body_bytes}) exceeds maximum ({MAX_JSON_BODY_BYTES})",
        )
        .into());
    }
    Ok(())
