...
impl HttpFilter for ModelRewriteFilter {
    ...
    async fn on_request_body(
        ...
        if !super::classifier::is_responses_create(&ctx.request.method, ctx.request.uri.path()) {
...
#[allow(clippy::struct_excessive_bools, reason = "independent decision flags")]
...
fn set_filter_results_from_result(ctx: &mut HttpFilterContext<'_>, result: &RewriteResult) {
    ...
    if !result.effective_model.is_empty()
        ...
        && result.effective_model.len() <= MAX_PROMOTED_VALUE_LEN
    {
        drop(results.set("effective_model", result.effective_model.clone()));
    }
    if result.rewritten {
        drop(results.set("rewritten", "true"));
    }
    if result.default_injected {
        drop(results.set("default_injected", "true"));
    }
}
...
fn repopulate_filter_results(ctx: &mut HttpFilterContext<'_>) {
    ...
    if let Some(eff) = effective {
        drop(results.set("effective_model", eff));
    }
    if rewritten {
        drop(results.set("rewritten", "true"));
    }
