...
#[tokio::test]
async fn stateless_notification_returns_202_no_body() {
    let filter = make_stateless_broker_filter();
    let mut req = crate::test_utils::make_request(http::Method::POST, "/mcp");
    req.headers.insert("content-type", "application/json".parse().unwrap());
    let mut ctx = crate::test_utils::make_filter_context(&req);
    ...
    match &action {
        FilterAction::Reject(rejection) => {
            assert_eq!(rejection.status, 202, "stateless notification should return 202");
            assert!(
                rejection.body.is_none(),
                "stateless notification should not include response body"
            );
        },
        _ => panic!("expected Reject with 202"),
    }
...
fn make_stateless_mcp_request(method: &str, mcp_name: Option<&str>) -> crate::context::Request {
    let mut req = crate::test_utils::make_request(http::Method::POST, "/mcp");
    req.headers.insert("content-type", "application/json".parse().unwrap());
    req.headers
        .insert("mcp-protocol-version", "2026-07-28".parse().unwrap());
