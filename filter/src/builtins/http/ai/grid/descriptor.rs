...
mod tests {
    ...
    fn valid_minimal_inference_candidate() {
        let candidates =
            validate_candidates(vec![candidate("inference_model", "llama", "site-a", "cluster-a")]).unwrap();
        ...
        assert_eq!(&*candidates[0].cluster, "cluster-a");
    ...
    fn mcp_tool_candidate_valid() {
        let candidates = validate_candidates(vec![candidate("mcp_tool", "weather", "site-b", "cluster-b")]).unwrap();
    ...
    fn a2a_agent_kind_rejected() {
        ...
        let err: Result<Vec<CandidateConfig>, _> = serde_yaml::from_str(yaml);
        assert!(err.is_err(), "a2a_agent is not a supported kind");
    }
