...
///
/// **Behavior:**
/// - If `ctx.cluster` is already set by an earlier filter, the selection
///   is preserved and no metadata is written.
/// - If the model header is absent, the filter returns `Continue` without
///   routing.
/// - If the header is blank, oversized, or non-UTF-8, the filter rejects
///   with 400.
/// - If a matching candidate is found, `ctx.cluster` is set and bounded
///   route-decision metadata is written.
...
mod tests {
    ...
    async fn oversized_model_header_rejects_no_metadata() {
        ...
        req.headers.insert("X-Model", http::HeaderValue::from_str(&big).unwrap());
        let mut ctx = crate::test_utils::make_filter_context(&req);
    ...
    async fn invalid_utf8_model_header_rejects_no_metadata() {
        let f = make_filter(&[("inference_model", "llama", "site-a", "inf")]);
        let mut req = crate::test_utils::make_request(Method::POST, "/chat");
        req.headers.insert("X-Model", http::HeaderValue::from_bytes(b"\xff\xfe").unwrap());
        let mut ctx = crate::test_utils::make_filter_context(&req);
    ...
    async fn unknown_model_rejects_404() {
        let f = make_filter(&[("inference_model", "llama", "site-a", "inf")]);
        let mut req = crate::test_utils::make_request(Method::POST, "/chat");
        req.headers.insert("X-Model", http::HeaderValue::from_static("unknown-model"));
        let mut ctx = crate::test_utils::make_filter_context(&req);
    ...
    fn assert_no_route_metadata(ctx: &HttpFilterContext<'_>) {
        assert!(ctx.get_metadata("grid.route.kind").is_none(), "grid.route.kind should be absent");
        assert!(ctx.get_metadata("grid.route.name").is_none(), "grid.route.name should be absent");
        assert!(ctx.get_metadata("grid.route.site").is_none(), "grid.route.site should be absent");
        assert!(ctx.get_metadata("grid.route.cluster").is_none(), "grid.route.cluster should be absent");
        assert!(ctx.get_metadata("grid.route.local_site").is_none(), "grid.route.local_site should be absent");
    }
