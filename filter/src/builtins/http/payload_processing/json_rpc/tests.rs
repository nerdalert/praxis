...
async fn extracts_notification() {
    ...
    assert!(!headers.contains_key("X-Json-Rpc-Id")); // Notifications don't have ids
...
async fn continues_on_incomplete_json() {
    ...
    let partial = br#"{"jsonrpc":"2.0","method":"test""#; // incomplete
...
async fn batch_rejection_overrides_default_on_invalid_continue() {
    // Test that batch_policy: reject works even with on_invalid: continue (the default)
    let filter = make_filter(); // Uses default on_invalid: continue
    let req = crate::test_utils::make_request(http::Method::POST, "/test");
    let mut ctx = crate::test_utils::make_filter_context(&req);
    ...
    // Should reject the batch despite default continue behavior for invalid input
    assert!(matches!(action, FilterAction::Reject(r) if r.status == 400));
}
...
async fn skips_header_with_control_chars() {
    ...
    // Should have kind header but not method header due to control chars
    let headers: std::collections::HashMap<_, _> =
        ctx.extra_request_headers.iter().map(|(k, v)| (k.as_ref(), v)).collect();
