...
fn method_based_routing_different_clusters() {
    ...
    // Test 1: MCP tools/call should go to backend1 (mcp-tools cluster)
    let raw1 = http_send(
        proxy.addr(),
        &json_post(
            "/mcp/",
    ...
    assert_eq!(body1, "mcp-tools-backend", "tools/call should route to MCP backend");
    ...
    assert_eq!(body2, "a2a-send-backend", "SendMessage should route to A2A backend");
}
...
/// Build a simple proxy YAML with `json_rpc` filter.
...
/// Build a routing proxy YAML with two backends for method-based routing.
