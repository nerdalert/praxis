...
use praxis_test_utils::{
    TestCertificates, free_port, http_get, https_get, registry_with, start_backend_with_shutdown,
    start_full_proxy, start_mtls_backend, start_proxy,
    start_tcp_echo_backend, start_tls_backend, start_tls_proxy, start_tls_proxy_no_wait, start_tls_proxy_no_wait_with_registry,
...
impl praxis_filter::HttpFilter for PeerIdentityRejectIfNone {
    ...
    async fn on_request(
        ...
        if ctx.peer_identity.is_some() {
            ...
            Ok(praxis_filter::FilterAction::Reject(praxis_filter::Rejection::status(403)))
        }
    }
}
...
// grid_ingress_trust integration tests
// -----------------------------------------------------------------------------

/// Verify that a client cert with a matching org is allowed through.
#[test]
fn grid_ingress_trust_allows_matching_org() {
    let certs = TestCertificates::generate();
...
"#,
    ...
    assert_eq!(
        status, 200,
        "grid_ingress_trust must allow cert with matching org (got {status}, body: {body})"
    );
...
#[test]
fn grid_ingress_trust_rejects_wrong_org() {
    let certs = TestCertificates::generate();
...
"#,
    ...
    assert_eq!(
        status, 403,
        "grid_ingress_trust must reject cert with non-matching org (got {status})"
    );
}

/// Verify that a connection without a client cert (request mode) is rejected by the filter.
#[test]
fn grid_ingress_trust_rejects_no_peer_identity() {
    let certs = TestCertificates::generate();
    let no_cert_config = certs.client_config();
